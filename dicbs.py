...
from single_agent_solver import a_star
...
def cbs_h2(agents, goals, heuristics, constraints_per_agent, problem):
    ...
    for agent_id, (agent, goal) in enumerate(zip(agents, goals)):
        # #print("initial path for agent ", agent_id, agent, goal)
        path, gVal, rhs, prev_cons,open_list = lpa_star(agent,goal,problem,agent_id, heuristics)
        if not path:
            return None
        # #print("path was found by lpa", prev_cons)
        paths.append(path)
        agent_states.append([deepcopy(gVal), deepcopy(rhs), deepcopy(prev_cons), deepcopy(open_list)])
    # #print("initial paths found by cbs_h2 ", paths, prev_cons)
    return (paths, agent_states)


def cbs_h2_a_star(agents, goals, heuristics):
    paths = []
    agent_states = []
    for agent_id, (agent, goal) in enumerate(zip(agents, goals)):
        path,gVal,rhs,prev_cons = a_star((agent,0),goal,heuristics[agent_id],agent_id,None,999,len(agents))
    ...
    #print("initial paths found by cbs_h2 a star ", paths)
    return paths, agent_states
...
def dicbs(agents, goals, heuristics, dynamic_obstacles,max_time,problem ,alpha=3,slpa=True): #agents is start loc of agents
    num_of_generated = 0      
    #print("calling cbs ", agents, goals) 
    ...
    def replan_path():
                    ...
                    #     return
                    if(slpa):
                        new_path, gVal, rhs, prev_cons, open_list = lpa_star(start_position[0],goals[agent_id], problem, agent_id, heuristics,childNode["constraints"][agent_id], True, childNode["agent_states"][agent_id],previous_path )
                    else:
                        new_path, gVal, rhs, prev_cons, open_list = a_star(start_position,goals[agent_id],heuristics[agent_id],agent_id,childNode["constraints"][agent_id],999,len(agents))
                    if new_path:
                            ...
                            #print(agent_id,  childNode)
                            childNode["agent_states"][agent_id] = [deepcopy(gVal), deepcopy(rhs), deepcopy(prev_cons), deepcopy(open_list)]
    ...
    if(slpa):
        ...
        root_paths, root_states = cbs_h2_a_star(agents,goals,heuristics)
    #print(root_states)
    ...
    while t <= max_time or len(node["collisions"]) > 0 or  detect_obstacle_collisions(node["paths"], env_cons) != []  :
        #print("Iteration: \n", t,  detect_obstacle_collisions(node["paths"], env_cons), t, len(node["collisions"]), max_time)
        affected_agents = set()
        ...
        for change in EC_t:
            if(change[1] in env_cons):
                env_cons[change[1]].append(change[0])
            else:
            ...
            for agent_id in range(len(agents)):
                if is_agent_affected(newNode["paths"][agent_id], EC_t):
                    ...
                    # #print("this agent is affected before", agent_id,agent_constraints)
                    if(change[1] in agent_constraints["negative"]['env']):
                        agent_constraints["negative"]['env'][change[1]].append(change[0])
                    else:
        ...
        for agent_id in range(len(agents)):
            # if(agent_id not in affected_agents):
                for cons in env_cons:
                    if((env_cons[cons],cons) == change and agent_id in affected_agents):
                        continue
                    agent_constraints = newNode["constraints"][agent_id]
    ...
    # # Find new paths for agents affected by env changes
        for agent_id in obstacle_colliding_agents:
            ...
            #print("prev path for ", agent_id,previous_path)
            #print("backtrack time ", backtrack_time)
            ...
            new_path = None
            if(slpa):
                new_path, gVal, rhs, prev_cons, open_list = lpa_star(start_position[0],goals[agent_id], problem, agent_id, heuristics,newNode["constraints"][agent_id], True, newNode["agent_states"][agent_id],previous_path )
            else:
                new_path, gVal, rhs, prev_cons = a_star(start_position,goals[agent_id], heuristics[agent_id],agent_id,newNode["constraints"][agent_id], 999,len(agents) )

            if new_path:
                #print("new path found for agent opo", agent_id , new_path, prev_cons)
                newNode["paths"][agent_id] = previous_path[:backtrack_time] + new_path
                newNode["agent_states"][agent_id] = [deepcopy(gVal),deepcopy(rhs),deepcopy(prev_cons), deepcopy(open_list)]
...
def push_node(open_list, node, num_of_generated):
        assert isinstance(node['cost'], (int, float)), "Cost must be a number"
        assert isinstance(len(node['collisions']), int)
